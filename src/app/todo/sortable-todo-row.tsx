...
export function SortableTodoRow({ item, handleEdit, deleteTodo, isDragDisabled }: SortableTodoRowProps) {
  ...
  return (
    <TableRow
      ref={setNodeRef}
      style={style}
      className={cn("hover:bg-muted/20 group bg-card", isDragging && "opacity-50 shadow-md")}
    >
      <TableCell className="font-medium flex items-center gap-2">
        {!isDragDisabled && (
          <button
            className="text-muted-foreground opacity-50 hover:opacity-100 cursor-grab active:cursor-grabbing p-1 -ml-2 rounded hover:bg-muted"
            {...attributes}
            {...listeners}
          >
